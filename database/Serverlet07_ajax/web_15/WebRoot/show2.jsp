...
<script type="text/javascript">
	...
	function go(){
		...
		if(cPage!=""){
			location.href="pageServlet?cPage="+cPage;
		}
	}
</script>
...
<tr>
	...
	当前是第${pb.currentPage}页
		...
		<c:if test="${pb.currentPage>1 }">
			<a href="pageServlet?cPage=${pb.currentPage-1}">上一页</a>
		</c:if>
		<c:forEach var="i" begin="1" end="${pb.totalPage}">
			<a href="pageServlet?cPage=${i}">${i}</a>
		</c:forEach>
		<c:if test="${pb.currentPage<pb.totalPage}">
			<a href="pageServlet?cPage=${pb.currentPage+1}">下一页 </a>
		 </c:if>
		<a href="pageServlet?cPage=${pb.totalPage}">尾页 </a>
		<input type="text" id="cPage" size="5"><a href="javascript:go()">GO</a>
	</td>
</tr>
</table>
	<table border="1" align="center" width="80%">
		...
		<c:if test="${empty pb.books }">
			<tr>
				<td colspan="8">
					<font color="red">木有符合条件的信息</font>
				</td>
			</tr>
		</c:if>
	</table>
